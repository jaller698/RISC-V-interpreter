object CheckerRules {
  ...
  def printBoard(board: Vector[Piece]) = {
    for (i <- 0 until 8) {
      for (y <- 0 until 4) {
        if (i % 2 == 0) {
          print("-")
  ...
  def movePiece(from: Int, to: Int, board: Vector[Piece]): Vector[Piece]={
    if(isMoveValid(from, to, board)){
        var nBoard=board
        nBoard(from)=Empty
        nBoard(to)=board(from)
        return nBoard
    }else{
        println("Not legal move")
        return board
    }
